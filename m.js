...
function stP() {
	mov=true;
			...
			te[0].p[te[0].act].vd=[Math.cos(te[0].p[te[0].act].t), Math.sin(te[0].p[te[0].act].t)];
			vload=false;
			turntimes+=1;
			if (turntimes==2) {
				turn=1;
...
function tos(x, y) {
	...
	for (i=0;i<te[0].p.length;i++) {
		...
		if (Math.sqrt(Math.pow(x-te[0].p[i].c[0], 2) + Math.pow(y-te[0].p[i].c[1], 2))<=te[0].p[i].r) {
			tpl=true;
			te[0].act=i;
			break;
		}
	}
	if (Math.sqrt(Math.pow(x-0.15*H, 2) + Math.pow(y-0.85*H, 2))<=0.08*H) {
		dir=-1;
	} else if (Math.sqrt(Math.pow(x-W+0.15*H, 2) + Math.pow(y-0.85*H, 2))<=0.08*H) {
		dir=1;
	} else if (!tpl) {
		stP2();
		loading=1;
...
var render = function () {
	...
	ctx.strokeStyle="rgba(0, 0, 0, 0)";
  	...
  	ctx.stroke();
  	ctx.beginPath();
  	ctx.arc(Math.floor(0.5*W), Math.floor(0.5*H), Math.floor(0.2*H), 0, 2*Math.PI);
  	ctx.stroke();
  	ctx.beginPath();
  	ctx.moveTo(2, 0.3*H);
  	ctx.lineTo(2, 0.7*H);
	...
	ctx.restore();
  	...
  	ctx.save();
  	ctx.beginPath();
  	var grd = ctx.createRadialGradient(ball.c[0],ball.c[1],ball.r*8/10,ball.c[0],ball.c[1],ball.r/10);
	...
	ctx.stroke();
  	...
  	for (i=0;i<2;i++) {
  		for (j=0;j<te[i].p.length;j++) {
  			ctx.save();
  			var grd = ctx.createRadialGradient(te[i].p[j].c[0],te[i].p[j].c[1],5,te[i].p[j].c[0],te[i].p[j].c[1],te[i].p[j].r+5);
 			switch(te[i].te) {
 				case 0:
  	...
  	if (turn==0) {
  		...
  		if (mov) {
			...
			ctx.fillStyle = 'rgba(255,255,255,0.3)';
	 		...
	 		ctx.moveTo(actP.c[0]+(actP.r+arrowH)*Math.cos(actP.t),actP.c[1]+(actP.r+arrowH)*Math.sin(actP.t));
	 		ctx.lineTo(actP.c[0]+(actP.r+5)*Math.cos(actP.t+0.3),actP.c[1]+(actP.r+5)*Math.sin(actP.t+0.3));
	 		ctx.lineTo(actP.c[0]+(actP.r+5)*Math.cos(actP.t-0.3),actP.c[1]+(actP.r+5)*Math.sin(actP.t-0.3));
...
function move(ctx) {
			...
			if (pl.v!=0) {
				...
				//Check collisions
					...
					//Check if collided with another player
						for (j=0; j<2; j++) {
							te[j].p.forEach(function(pl2) {
								if (pl.c != pl2.c && Math.sqrt(Math.pow(pl.c[0]-pl2.c[0], 2) + Math.pow(pl.c[1]-pl2.c[1], 2))<pl.r+pl2.r) {
									...
									pl.vd=[pl.vd[0]*Math.cos(angle) + pl.vd[1]*Math.sin(angle), -pl.vd[0]*Math.sin(angle) + pl.vd[1]*Math.cos(angle)]
					...
					//Check if collided with ball
						if (pl.c != ball.c && Math.sqrt(Math.pow(pl.c[0]-ball.c[0], 2) + Math.pow(pl.c[1]-ball.c[1], 2))<pl.r+ball.r) {
	...
	//Check if ball collided with player
						for (j=0; j<2; j++) {
							te[j].p.forEach(function(pl2) {
								if (ball.c != pl2.c && Math.sqrt(Math.pow(ball.c[0]-pl2.c[0], 2) + Math.pow(ball.c[1]-pl2.c[1], 2))<ball.r+pl2.r) {
									c1c2=[ball.c[0]-pl2.c[0], ball.c[1]-pl2.c[1]];
									//TO DO: CALCULATE ANGLE BETWEEN c1c2 AND PLAYER'S V. MAKE (90º-ANGLE)*2 AND ROTATE.
									angle=Math.atan2(c1c2[0], c1c2[1])-Math.atan2(ball.vd[0], ball.vd[1]);
									ball.vd=[ball.vd[0]*Math.cos(angle) + ball.vd[1]*Math.sin(angle), -ball.vd[0]*Math.sin(angle) + ball.vd[1]*Math.cos(angle)]
...
function move2() {
	...
	if (turn==1&&mov) {//Move the other team
		turn=2;
		setTimeout(function() {
			turn=1;
			turntimes+=1;
			...
			if (Math.random()<0.3) {
				...
				norm=Math.sqrt(Math.pow(te[1].p[wp].vd[0], 2) + Math.pow(te[1].p[wp].vd[1], 2));
				...
				te[1].p[wp].v=Math.random()*20+60;	
				te[1].p[wp].vd=[ball.c[0]-te[1].p[wp].c[0], ball.c[1]-te[1].p[wp].c[1]];	
